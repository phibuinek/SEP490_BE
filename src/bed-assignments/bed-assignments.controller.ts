...
export class BedAssignmentsController {
  ...
  async getByResident(
    ...
    console.log('Bed assignment check - req.user:', JSON.stringify(req.user, null, 2));
    
    if (userRole === Role.FAMILY) {
      // Kiểm tra resident này có thuộc về family không
      const resident = await this.residentsService.findOne(resident_id);
      console.log('Bed assignment check - resident found:', resident ? 'yes' : 'no');
      console.log('Bed assignment check - resident:', JSON.stringify(resident, null, 2));
      console.log('Bed assignment check - resident.family_member_id:', resident?.family_member_id);
      console.log('Bed assignment check - resident.family_member_id.toString():', resident?.family_member_id?.toString());
      console.log('Bed assignment check - userId:', userId);
      console.log('Bed assignment check - comparison result:', resident?.family_member_id?.toString() === userId);
      ...
      console.log('Bed assignment check - familyMemberIdStr:', familyMemberIdStr);
      console.log('Bed assignment check - userIdStr:', userIdStr);
      console.log('Bed assignment check - final comparison:', familyMemberIdStr === userIdStr);
  ...
  async debugFamilyResidents(@Req() req) {
    ...
    try {
      const residents = await this.residentsService.findAllByFamilyMemberId(userId);
      ...
      return {
        ...
        residents: residents.map(r => ({
          ...
          family_member_id_str: typeof r.family_member_id === 'object' && r.family_member_id?._id ? 
            r.family_member_id._id.toString() : (r.family_member_id as any)?.toString()
        }))
      };
    } catch (error) {
  ...
  async debugBedAssignment(@Param('residentId') residentId: string, @Req() req) {
    const userRole = req.user?.role;
    const userId = req.user?.userId;
